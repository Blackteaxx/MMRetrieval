...
class Visualized_BGE(nn.Module):
    def __init__(
        ...
        if not from_pretrained:
            ...
            bge_config = AutoConfig.from_pretrained(
        ...
        if not from_pretrained:
            self.tokenizer = AutoTokenizer.from_pretrained(
                model_name_bge, use_fast=False
            )
        else:
            self.tokenizer = AutoTokenizer.from_pretrained(
                from_pretrained, use_fast=False
            )
    ...
    def encode_mm(self, images: torch.Tensor, texts):
        ...
        img_position_embeddings = self.bge_embeddings.position_embeddings(
            img_token_position_ids
        )
        ...
        prompt_img_embedding = torch.cat(
            [cls_token, img_token_emb, prompt_embedding_output], dim=1
        )

        img_attention_mask = torch.ones(batch_size, img_token_len, device=device)
        prom_img_attention_mask = torch.cat(
            [img_attention_mask, prompt_attention_mask], dim=1
        )
